...
class RuleLoader {
   ...
   * @param {object} context   — { docPath, workspaceRoot, wrapperRoot, vacuumDiags, cache, text }
  ...
  async runScripts(doc, context) {
    ...
    const perScriptContext = {
      ...
      wrapperRoot: context.wrapperRoot,  // ADR-0002: path to wrapper for dep resolution
      vacuumDiags: context.vacuumDiags || [],
      cache: sharedCache,
      text: context.text,  // raw YAML/JSON text — for exact-range scripts
    };
